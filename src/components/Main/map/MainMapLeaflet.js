...
class MainMapLeaflet extends React.Component {
	constructor(props) {
		super(props)
		...
		if (this.props.onFunctions) {
			const functions = {
				map: {
					...
					flyToBounds: (...attr) => this.map ? this.map.flyToBounds(...attr) : null,
					setView: (...attr) => this.map ? this.map.setView(...attr) : null,
					...
					invalidateSize: (...attr) => this.map ? this.map.invalidateSize(...attr) : null,
					project: (...attr) => this.map ? this.map.project(...attr) : null,
					unproject: (...attr) => this.map ? this.map.unproject(...attr) : null,
					latLngToContainerPoint: (...attr) => this.map ? this.map.latLngToContainerPoint(...attr) : null,
...
export default withConicGradient(withGlobals(withLocalization(withTheme(MainMapLeaflet))))
